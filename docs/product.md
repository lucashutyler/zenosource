...
- **Purchase Orders** — creation/import, defined lifecycle (draft → issued → acknowledged → in-progress → fulfilled/closed), with supplier rejection and buyer cancellation as explicit exits at any point before fulfillment. Line and release-level tracking, supplier-driven change proposals (qty/price/date) with buyer accept/reject. Each line specifies a delivery **location**.
- **Locations** — buyer-side delivery locations, comparable to (but simpler than) Epicor's Company → Site structure: a ZenoSource tenant spans all of a buyer's Epicor Companies, and `Location` is the only sub-tenant scope we model, standing in for an Epicor Site regardless of which Company it belongs to. Every PO line specifies a delivery location; internal users are assigned to specific locations and only manage POs destined for those locations. See [docs/data-model.md](data-model.md#location).
...
- **Minimizing external friction is part of being opinionated, not a contradiction of it.** The restrictive posture applies to how the buyer-side workflow is shaped — not to how hard it is for a supplier to respond. External users should be able to resolve an open action item (acknowledge a PO, answer an RFQ, confirm a date) as directly as clicking straight through from the reminder email into a scoped action view, with no account creation and no login required. If they have access to the email, that's sufficient proof they're the right contact to respond. Persistent supplier accounts (password login to see everything they owe) are deferred out of v1 — the reminder digest is the supplier's aggregate view for now, and if accounts are ever added they must never become a precondition for clearing a single action item. See [docs/architecture.md](architecture.md) for how this fits the auth model.
- **Action-driven doesn't mean inbox-only.** The action-item mechanism is what makes sure nothing sits forgotten, not the only way to interact with the system. Buyers still need ordinary list views of POs, RFQs, price lists, and suppliers — filterable, sortable, viewable, and (within what the state machine allows) editable and duplicable, e.g. copying an existing PO's supplier and lines into a new draft for a repeat order. The restriction is on inventing new statuses or skipping steps, not on browsing and managing the data.
