...
- **The connector contract is `contract.ts`**, stated entirely in ZenoSource's vocabulary — with `idp-contract.ts` beside it since Phase 3, because an ERP that pulls batches outward and an identity provider that is *called* by a directory have almost nothing in common, and folding them into one type whose members are mostly optional is the same as having no contract at all. An integration subproject depends on nothing in the platform and restates the canonical shapes structurally; the platform depends on the integration. `conformance.test.ts` turns any drift between the two into a build failure, and `vocabulary.test.ts` fails the build if a vendor's protocol vocabulary — a business-object name, a schema identifier, an assertion element, a token wire parameter — appears anywhere in `apps/platform/src` at all.
- **Identity is gated by the same registry, but sign-in deliberately does not read it.** `sso_oidc`, `sso_saml` and `scim_provisioning` are capabilities like any other and appear in the same "what's switched on" list. What they must *not* gate is signing in, because `DEGRADED` grants nothing — and the reasoning behind that rule is the harm of stale mirrored data, which a signature-verified assertion presented seconds ago does not have. Meanwhile the harm of withdrawal inverts: it would lock every user of a tenant out, including the owner who is the only person who could repair it. So the broker reads the connection row directly. This is written down in `capabilities.ts` where somebody about to make it "consistent" will read it.
...
- **Internal users** belong to exactly one tenant and can authenticate two ways: a password, or that tenant's federated IdP once one is connected (Okta, built in Phase 3). Federation is an *additional* door and never replaces the password — every safe version of "SSO required" needs either outbound email or a new pre-issued credential class, and both are later phases; see [docs/todo.md](todo.md). A tenant is resolved before any credential is validated: by verified email domain when a sign-in starts, by URL path segment when one comes back. Neither reads the credential, because a document must never nominate the keys used to trust it.
- **A user has a status.** Deactivation is a real state that the DAL reads on every request, which is what makes "the directory disabled Casey" take effect on Casey's next request even though sessions are stateless signed cookies. It also always hands their open work to somebody — a person names a successor, a directory event gets the tenant's oldest active owner — because an item owned by somebody who left looks fine to everyone else, every count on the board being scoped to its owner.
...
- Auth is split by population: internal users authenticate via the tenant's federated SSO; external users resolve action items exclusively through scoped, no-login action-view links (see Action items & reminders below). Persistent supplier accounts — password login, a supplier-facing portal — are deliberately deferred out of v1 (decided 2026-07-28, [docs/todo.md#decided](todo.md#decided)); the reminder digest is a supplier's aggregate view of everything they owe. Internal sessions and scoped action-view grants still funnel into one authorization layer — a grant just carries much narrower authority than a session. Protocol and provisioning detail: [docs/integrations.md#okta-idp](integrations.md#okta-idp).
