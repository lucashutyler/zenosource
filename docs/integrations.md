...
- **Internal vs. external users**: internal (buyer-org) users authenticate via the tenant's federated Okta (SSO). External (supplier) users generally have no enterprise IdP relationship with the buyer and authenticate via ZenoSource's own app-native auth. Both land behind one session/authorization layer so the rest of the platform is agnostic to which path a user came in through.
